...
final class InsightService {
    ...
    #if canImport(FoundationModels)
    @available(iOS 26, *)
    private func generateWithAppleIntelligence(stats: StatsSnapshot, language: AppLanguage) async -> String? {
        let languageName: String
        switch language {
        case .english: languageName = "English"
        case .spanish: languageName = "Spanish (Español)"
        case .french:  languageName = "French (Français)"
        }
    ...
    private func buildFallbackSpanish(stats: StatsSnapshot) -> String {
        ...
        if let step = stats.dropOffStep, let pct = stats.dropOffPct, pct >= 10 {
            ...
            let moodCompleted = Int((Double(moodPct) / 100.0 * Double(moodTotal)).rounded())
            parts.append("Solo terminas \(moodCompleted) de \(moodTotal) nudges (\(moodPct)%) cuando te sientes \(mood). Completa el Paso 1 en el momento en que aparezca ese estado, antes de que la evasión tome el control.")
        } else {
            let rate = stats.completionRatePct
            if rate < 50 {
                parts.append("Tu tasa de finalización es del \(rate)%. Haz el Paso 1 justo después de crear un nudge mientras tu intención aún está fresca.")
            } else {
                parts.append("Completas el \(rate)% de tus nudges. Hacer nudges a la misma hora cada día elevará aún más esa tasa.")
            }
        }
        ...
        if let friction = stats.topFrictionLabel,
           ...
           let energyPct = stats.bestEnergyCompletionPct {
            parts.append("\"\(friction)\" representa el \(frictionPct)% de tus etiquetas de fricción y completas el \(energyPct)% de tareas con energía \(energy), así que reserva esa ventana para tus sesiones más difíciles.")
        } else if let friction = stats.topFrictionLabel, let frictionPct = stats.topFrictionPct {
            parts.append("\"\(friction)\" es tu principal bloqueador con un \(frictionPct)% de los nudges. Nómbralo antes de empezar y la resistencia se sentirá menor.")
        } else if let energy = stats.bestEnergy, let energyPct = stats.bestEnergyCompletionPct {
            parts.append("Completas el \(energyPct)% de las tareas con energía \(energy). Reserva esa ventana para las tareas que más llevas evitando.")
        }
    ...
    private func buildFallbackFrench(stats: StatsSnapshot) -> String {
        ...
        if let step = stats.dropOffStep, let pct = stats.dropOffPct, pct >= 10 {
            parts.append("\(pct)% de tes nudges stagnent à l'Étape \(step). Lance l'Étape \(step + 1) avant de fermer l’app et ce schéma se brisera.")
        } else if let mood = stats.topMood, let moodPct = stats.topMoodCompletionPct, let moodTotal = stats.topMoodTotalCount {
            let moodCompleted = Int((Double(moodPct) / 100.0 * Double(moodTotal)).rounded())
            parts.append("Tu ne termines que \(moodCompleted) nudges sur \(moodTotal) (\(moodPct)%) quand tu te sens \(mood). Lance l’Étape 1 dès que cet état apparaít, avant que l’évitement ne prenne le dessus.")
        } else {
            let rate = stats.completionRatePct
            if rate < 50 {
                ...
                parts.append("Tu complètes \(rate)% de tes nudges. Faire des nudges à la même heure chaque jour poussera ce taux encore plus haut.")
            }
        }
        ...
        if let friction = stats.topFrictionLabel,
           ...
           let energy = stats.bestEnergy,
           let energyPct = stats.bestEnergyCompletionPct {
            parts.append("«\(friction)» représente \(frictionPct)% de tes étiquettes de friction et tu complètes \(energyPct)% des tâches avec une énergie \(energy), donc réserve cette fenêtre pour tes sessions les plus difficiles.")
        } else if let friction = stats.topFrictionLabel, let frictionPct = stats.topFrictionPct {
            parts.append("«\(friction)» est ton principal bloqueur avec \(frictionPct)% des nudges. Nomme-le avant de commencer et la résistance paraître plus petite.")
        } else if let energy = stats.bestEnergy, let energyPct = stats.bestEnergyCompletionPct {
            parts.append("Tu complètes \(energyPct)% des tâches avec une énergie \(energy). Réserve cette fenêtre pour les tâches que tu évites depuis le plus longtemps.")
        }
